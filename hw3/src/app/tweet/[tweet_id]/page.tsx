...
export default async function TweetPage({
  ...
  const [tweetData] = await db
    .select({
      ...
      selectedFromDate: tweetsTable.selectedFromDate,
      selectedToDate: tweetsTable.selectedToDate,
  ...
  const tweet = {
    ...
    selectedFromDate: tweetData.selectedFromDate,
    selectedToDate: tweetData.selectedToDate,
      ...
      <div className="flex h-screen w-full flex-col overflow-scroll pt-2">
        ...
        <div className="flex flex-col px-4 pt-3">
          ...
          <time className="my-4 block text-sm text-gray-500">
            From:
            <TimeText date={tweet.selectedFromDate} format="YYYY-M-D h" />
            <div></div>
            To: 
            <TimeText date={tweet.selectedToDate} format="YYYY-M-D h" />
          </time>
          <Separator />
          <div className="my-2 flex items-center justify-between gap-4 text-gray-400">
            ...
            <div className={"flex items-center gap-1 rounded-full p-3 duration-300 text-brand"}>
              參加人數 : {tweet.likes}
            </div>
            {/* <button className="rounded-full p-1.5 transition-colors duration-300 hover:bg-brand/10 hover:text-brand">
              <Share size={18} />
            </button> */}
            {!liked && <div className={"flex items-center gap-1 rounded-full p-3 duration-300 text-brand"}>
              參加活動即可加入討論
            </div>}
          </div>
          <Separator />
        ...
        {replies.slice().reverse().map((reply) => (
          <Tweet
            ...
            username={username}
            handle={handle}
            selectedFromDate={reply.selectedFromDate}
            selectedToDate={reply.selectedToDate}
