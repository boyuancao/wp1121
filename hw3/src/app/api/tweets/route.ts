...
const postTweetRequestSchema = z.object({
  ...
  selectedFromDate: z.string().min(1).max(50),
  selectedToDate: z.string().min(1).max(50),
...
export async function POST(request: NextRequest) {
  ...
  const { handle, content, selectedFromDate, selectedToDate, replyToTweetId } = data as PostTweetRequest;
