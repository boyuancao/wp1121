  ...
  const onSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (location.pathname === '/login') {
      ...
      if (password !== confirmPassword) {
        // Passwords do not match, show a toast with the message
        const toastMessage = 'Passwords do not match';
        toast({ description: toastMessage, variant: 'destructive' });
        ...
        <CardHeader>
          <CardTitle className="flex items-center gap-2 tracking-normal">
            ...
            <div className="flex items-center">
              ...
              <h1 data-testid="header-title" className="ml-2">VSCoddit</h1>
            </div>
            {/* END of TODO 1.1 */}
          </CardTitle>
          <CardDescription>
          ...
          <div className="grid w-full items-center space-y-4">
            <div className="flex flex-col gap-2">
              ...
              <Input
                ...
                value={username}  // Assume you have a `username` state
                onChange={(e) => setUsername(e.target.value)}  // Control the input with `username` state
              />
              {/* End of TODO 1.4 */}
            </div>
            <div className="flex flex-col gap-2">
              ...
              <Input
                ...
                required  // Required attribute
                value={password}  // Assume you have a `password` state
                onChange={(e) => setPassword(e.target.value)}  // Control the input with `password` state
              />
              {/* End of TODO 1.4 */}
            </div>
            <div
            ...
            >
              ...
              <Input
                ...
                required={location.pathname !== '/login'}  // Required only if in the register page
                value={confirmPassword}  // Assume you have a `confirmPassword` state
                onChange={(e) => setConfirmPassword(e.target.value)}  // Control the input with `confirmPassword` state
              />
              {/* End of TODO 1.5 */}
            </div>
          </div>
